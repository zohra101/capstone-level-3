...
export function handleSubmitCall(event) {
	...
	const promise = new Promise(callServerResponse); //Initiates a new promise
	...
	promise.then(callParseResponse).then(function hideSpinner(resolveValue2) {//Attaches callParseResponse as the argument/callback function to be called when callServerResponse is fulfilled. When callParseResponse is fulfilled, hideSpinner is the new promise. 
